...
class MultiHeadAttention(nn.Module):
    ...
    def __call__(self, x: jax.Array, mask: jax.Array, deterministic:bool =None):
        ...
        qkv = nn.Dense(3 * C, name="qkv", use_bias=self.use_bias)(x)  # [B, T, 3 * C]
        qkv = jnp.reshape(qkv, (B, T, 3 * self.num_heads, head_dim))  # [B, T, 3 * H, D]
        k,q,v = jnp.split(qkv, 3, axis=2)   # [B, T, H, D]

        # Calculate self attention matrix.
        attn = jnp.einsum("bthd,bThd->bhtT", q, k) / jnp.sqrt(head_dim).astype(self.dtype)
        attn = jnp.where(mask, attn, jnp.finfo(attn.dtype).min)
        attn = jax.nn.softmax(attn, axis=-1).astype(self.dtype)
        attn = nn.Dropout(self.dropout_rate)(attn, deterministic)   # [B, H, T, T]
        ...
        x = nn.Dense(features=C, name="out_proj", dtype=self.dtype, use_bias=self.use_bias)(x)  # [B, T, C]
...
class Block(nn.Module):
    ...
    def setup(self):
        ...
        self.attn = MultiHeadAttention(self.num_heads, self.dtype, dropout_rate=self.dropout_rate) 
        self.post_attn_ln = nn.LayerNorm(epsilon=self.ln_epsilon, dtype=self.dtype, use_bias=self.use_bias)
        self.mlp = MLP()
